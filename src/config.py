...
class Settings:
    ...
    def __post_init__(self):
        ...
        self.temperature = self.temperature or float(_get_env_or_yaml(
            "TEMPERATURE", llm_cfg.get("parameters", {}).get("temperature"), 0.1
        ))
        self.max_tokens = self.max_tokens or int(_get_env_or_yaml(
        ...
        self.retrieval_k = self.retrieval_k or int(_get_env_or_yaml(
            "RETRIEVAL_K", agent_cfg.get("retrieval_k"), 5
        ))
        self.chunk_size = self.chunk_size or int(_get_env_or_yaml(
            "CHUNK_SIZE", agent_cfg.get("chunk_size"), 1000
        ))
        self.chunk_overlap = self.chunk_overlap or int(_get_env_or_yaml(
            "CHUNK_OVERLAP", agent_cfg.get("chunk_overlap"), 200
        ))
        self.confidence_threshold = self.confidence_threshold or float(_get_env_or_yaml(
