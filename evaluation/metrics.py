...
def check_snippet_presence(answer: str, expected_snippets: list[str]) -> float:
    """
    ...
    found = sum(1 for snippet in expected_snippets if snippet in answer)
...
def calculate_aggregate_metrics(
    results: list[EvaluationResult], questions_metadata: dict[str, dict[str, Any]]
) -> AggregateMetrics:
    """
    ...
    return AggregateMetrics(
        ...
        avg_aggregate_score=safe_avg(
            [calculate_aggregate_score(r) for r in successful]
        ),
        ...
        scores_by_difficulty={
            k: sum(v) / len(v) for k, v in scores_by_difficulty.items()
        },
    )
